      ...
      (async () => {
        try {
            ...
            if (node.directory) {
              ...
              btn.onclick = async () => {
                ...
                try {
                  ...
                  alert('Download failed — try opening the link manually.');
                } finally {
                  clearInterval(anim);
                  btn.textContent = 'Download';
                  btn.disabled = false;
          ...
          for (const child of folder.children) {await walk(child);}
        } catch (err) {console.error(err);document.getElementById('file-list').innerHTML = ' < li style = "color:#b91c1c; text-align:center;" > Error loading folder– check the console. < /li>';}})();
